...
def demo(args):
    ...
    dummy_input_left = torch.randn(1, 3, 384, 640).to(DEVICE).float()  # 左图像
    dummy_input_right = torch.randn(1, 3, 384, 640).to(DEVICE).float()  # 右图像

    # dummy_input_left = torch.randn(1, 3, 544, 960).to(DEVICE).float()  # 左图像
    # dummy_input_right = torch.randn(1, 3, 544, 640).to(DEVICE).float()  # 右图像
    ...
    import onnxsim
    try:
        model_onnx = onnx.load("IGEVStereo.onnx")  # load onnx model
        ...
        onnx.save(model_opt, "IGEVStereo_simplified.onnx")
        print("ok")
    except Exception as e:
